(function attachStepDefinitions(root, factory) {
  ...
  function createCodex2ApiSessionImportTail(startId, startOrder) {
    const id = Number(startId) || 10;
    const order = Number(startOrder) || id * 10;
    return [{
      id,
      order,
      key: 'codex2api-session-import',
      title: '导入当前 ChatGPT 会话到 Codex2API',
      sourceId: 'codex2api-panel',
      driverId: 'background/codex2api-session-import',
      command: 'codex2api-session-import',
    }];
  }
  ...
  function resolvePlusSessionImportTail(options = {}, signupMethod = SIGNUP_METHOD_EMAIL) {
    ...
    if (strategy === PLUS_ACCOUNT_ACCESS_STRATEGY_CODEX2API_CODEX_SESSION) {
      return createCodex2ApiSessionImportTail;
    }
    return null;
  }
  ...
  const PLUS_PAYPAL_CODEX2API_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    PLUS_PAYPAL_PREFIX_STEP_DEFINITIONS,
    10,
    100,
    SIGNUP_METHOD_EMAIL,
  ...
  const PLUS_PAYPAL_HOSTED_CHECKOUT_CODEX2API_SESSION_STEP_DEFINITIONS = createHostedCheckoutSteps(
    PLUS_PAYPAL_HOSTED_CHECKOUT_PREFIX_STEP_DEFINITIONS,
    7,
    70,
    SIGNUP_METHOD_EMAIL,
  ...
  const PLUS_GOPAY_CODEX2API_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    PLUS_GOPAY_PREFIX_STEP_DEFINITIONS,
    10,
    100,
    SIGNUP_METHOD_EMAIL,
  ...
  const PLUS_GPC_CODEX2API_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    PLUS_GPC_PREFIX_STEP_DEFINITIONS,
    10,
    100,
    SIGNUP_METHOD_EMAIL,
  ...
  function getOpenAiModeStepDefinitions(options = {}) {
    ...
    if (shouldTreatHostedCheckoutAsFinalStep({
      ...
      if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_CODEX2API_CODEX_SESSION) {
        return PLUS_PAYPAL_HOSTED_CHECKOUT_CODEX2API_SESSION_STEP_DEFINITIONS;
      }
    ...
    openai: {
      getAllSteps() {
        ...
        for (const step of [
          ...
          ...PLUS_PAYPAL_HOSTED_CHECKOUT_CODEX2API_SESSION_STEP_DEFINITIONS,
          ...PLUS_PAYPAL_HOSTED_CHECKOUT_PREFIX_STEP_DEFINITIONS,
          LOCAL_CPA_JSON_NO_RT_EXPORT_STEP_DEFINITION,
          ...PLUS_PAYPAL_HOSTED_CHECKOUT_PHONE_STEP_DEFINITIONS,
          ...PLUS_PAYPAL_HOSTED_CHECKOUT_PHONE_BOUND_EMAIL_RELOGIN_STEP_DEFINITIONS,
